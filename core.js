...
window.addEventListener("wheel", event=>{
	...
	player.impulse.y = Math.max(player.impulse.y - event.deltaY*0.1, 0); // cannot go below 0
	...
	player.torqueImpulse.z -= event.deltaX*0.5; // roll correction via mouse wheel
});
...
function createPlayer(){
	...
	//threeJS Section
	player = new THREE.Mesh(new THREE.BoxBufferGeometry(), new THREE.MeshPhongMaterial({color: 0xff0505}));
